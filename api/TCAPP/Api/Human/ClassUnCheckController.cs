...
namespace HrApp.Api
{
    ...
    public class ClassUnCheckController : Controller
    {
        ...
        public JsonResult Query(string groupId, string storeId, string keyword, int? pageNumber, int? pageSize)
        {
            ...
            try
            {
                ...
                condition.Add(new SimpleCondition("State", Convert.ToInt16(Enum_MakeClassState.Pass)));
        ...
        public JsonResult UnCheck(int id)
        {
            CustomJsonResult json = new CustomJsonResult();
            json.JsonRequestBehavior = JsonRequestBehavior.AllowGet;
            json.ContentType = "text/plain";
            try
            {
                ...
                if (plan != null)
                {
                    if (plan.State == Convert.ToInt16(Enum_MakeClassState.Pass))
                    {
                        ...
                        log.SourceState = Convert.ToInt16(Enum_MakeClassState.Pass);
                        ...
                        json.Data = JsonUtil.GetSuccessForString("反审核成功");
                    }
                    else
                    {
