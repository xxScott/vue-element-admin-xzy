    ...
    public class CheckController : Controller
    {
        public JsonResult Query(int storeId, int peroidId, string keyword, int? pageNumber, int? pageSize)
        {
            ...
            try
            {
                ...
                int totalCount = 0;
                DataView dv = service.QueryAssess(storeId, Convert.ToInt16(Enum_AssessState.Wait), peroidId, keyword, pageNumber, pageSize, out totalCount).DefaultView;
                //数据转换，查询枚举描述
        ...
        public JsonResult Check(int id, int checkState)
        {
            ...
            try
            {
                ...
                if (assess != null)
                {
                    if (assess.State == Convert.ToInt16(Enum_AssessState.Wait))
                    {
