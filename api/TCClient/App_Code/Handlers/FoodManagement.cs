...
namespace HrApp.TCClient.Handlers
{
            ...
            try
            {
                using (TCDataContextDataContext context = MyDataContextFactory.GetDataContext<TCDataContextDataContext>(MyDataContextFactory.CaiMoMo_TC))
                {
                    ...
                    if (!object.Equals(tc_Food, null))
                    {
                        ...
                        //tc_Food.UpdateUser = currentUser.TrueName;
    ...
    public class TcFoodManagementQueryList : IAjaxHandler
    {
        ...
        public HandleResult HandleRequest(HttpContext hContext, NameValueCollection queryString, string postContent)
        {
            try
            {
                using (TCDataContextDataContext context = MyDataContextFactory.GetDataContext<TCDataContextDataContext>(MyDataContextFactory.CaiMoMo_TC))
                {
                    ...
                    #region 每次登录时更新OpenID
                    string strOpenID = currentUser.OpenID;
                    string strMsg = "";
                    if (!updateLoginOpenID(strOpenID, (int)groudid, (int)storeid, ref strMsg))
                    {
        ...
        /// <returns></returns>
        private bool updateLoginOpenID(string openid, int groudid, int storeid, ref string strmsg)
        {
            try
            {
                using (TCDataContextDataContext context = MyDataContextFactory.GetDataContext<TCDataContextDataContext>(MyDataContextFactory.CaiMoMo_TC))
                {
                    TC_UserAuths ua = context.TC_UserAuths.SingleOrDefault(g => g.Identifier == openid);
                    if (ua != null)
                    {
                        ua.GroupID = groudid;
                        ua.StoreID = storeid;
                        ua.LastLoginTime = System.DateTime.Now;
                    ...
                    else
                    {
                        ...
                        tC_UserAuths.GroupID = groudid;
                        tC_UserAuths.StoreID = storeid;
                        tC_UserAuths.Identifier = openid;
                        tC_UserAuths.LastLoginTime = System.DateTime.Now;
            ...
            {
                Log.WriteLog("更新OpenID登录状态出错:" + ex);
                strmsg = "更新OpenID登录状态出错";
                return false;
            }
        }
    }
